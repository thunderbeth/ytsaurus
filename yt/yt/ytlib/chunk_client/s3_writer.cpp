...
public:
    TS3UploadSessionBase(
        ...
        TS3MediumStub::TS3ObjectPlacement objectPlacement,
...
protected:
    ...
    const TS3MediumStub::TS3ObjectPlacement ObjectPlacement_;
    const NLogging::TLogger Logger;
...
{
public:
    //! This is a minimum enforced by Amazon S3 specification.
    static constexpr i64 MinPartSize = 5_MB;

    struct TOptions
    {
        i64 PartSize = MinPartSize;
        i64 UploadQueueSizeLimit = 100_MB;
    };
    ...
    TS3MultiPartUploadSession(
        ...
        , UploadQueueSemaphore_(New<TAsyncSemaphore>(Options_.UploadQueueSizeLimit))
    {
        ...
        YT_VERIFY(Options_.PartSize >= MinPartSize);

        YT_LOG_DEBUG(
            "Created multi-part upload session (MinPartSize: %v, UploadQueueSizeLimit: %v)",
            Options_.PartSize,
            Options_.UploadQueueSizeLimit);
    }
    ...
    bool Add(const std::vector<TSharedRef>& data)
    {
        ...
        UploadQueueSemaphore_->Acquire(size);
        ...
        return UploadQueueSemaphore_->IsReady();
    }
    ...
    TFuture<void> GetReadyEvent()
    {
        ...
        promise.TrySetFrom(UploadQueueSemaphore_->GetReadyEvent());
...
private:
    ...
    TAsyncSemaphorePtr UploadQueueSemaphore_;
    ...
    void DoStart()
    {
        ...
        // TODO(achulkov2): Cancel this future if the session is aborted.
    ...
    void GuardedSchedulePartUploadIfNeeded()
    {
        ...
        YT_LOG_DEBUG(
            "KEK Thinking about scheduling part upload (IsAborted: %v, State: %v, BufferedDataVectorSize: %v, BufferedDataSize: %v, PartSize: %v)",
            IsAborted(),
            State_,
            BufferedData_.size(),
            BufferedDataSize_,
    ...
    NS3::TUploadPartResponse OnPartUploadCompleted(const TErrorOr<NS3::TUploadPartResponse>& response, int partIndex, i64 partSize)
    {
        ...
        if (response.IsOK()) {
            ...
            UploadQueueSemaphore_->Release(partSize);
    ...
    void DoComplete()
    {
        ...
        // TODO(achulkov2): Separate into method?
        ...
        // TODO(achulkov2): Thread-safe access to UploadId_?
        // TODO(achulkov2): Cancel this future if the session is aborted.
...
public:
    TS3Writer(
        ...
        , ChunkPlacement_(Medium_->GetChunkPlacement(SessionId_.ChunkId))
        ...
        , ChunkUploadSession_(New<TS3MultiPartUploadSession>(
            ...
            TS3MultiPartUploadSession::TOptions{
                ...
                .UploadQueueSizeLimit = Config_->UploadQueueSizeLimit,
            },
            TDispatcher::Get()->GetWriterInvoker(),
            Logger))
        , ChunkMetaUploadSession_(New<TS3SimpleUploadSession>(
            Client_,
            TS3MediumStub::GetChunkMetaPlacement(ChunkPlacement_),
            TDispatcher::Get()->GetWriterInvoker(),
            Logger))
    { }
    ...
    {
        // TODO(achulkov2): Check state?
        auto writeRequest = ChunkLayoutFacade_->AddBlocks(blocks);

        YT_LOG_DEBUG("KEK Writing block (StartOffset: %v, EndOffset: %v, CurrentOffset: %v)", writeRequest.StartOffset, writeRequest.EndOffset, ChunkUploadSession_->GetCurrentOffset());

        Cerr << Format("KEK Writing block (StartOffset: %v, EndOffset: %v, CurrentOffset: %v)", writeRequest.StartOffset, writeRequest.EndOffset, ChunkUploadSession_->GetCurrentOffset());
        Cerr << Endl;

        // We should be the only ones adding data, and it should be written contiguously :)
        // YT_VERIFY(writeRequest.StartOffset == ChunkUploadSession_->GetCurrentOffset());
        ...
        const TDeferredChunkMetaPtr& chunkMeta = nullptr) override
    {
        ...
        // TODO(achulkov2): Maybe we need to throw instead? And/or forbid them on some other level.
        YT_VERIFY(chunkMeta);
        ...
        // TODO(achulkov2): Maybe we want to upload meta multiple times to increase the speed of
        // reading a single chunk in parallel?
    ...
    const NChunkClient::NProto::TDataStatistics& GetDataStatistics() const override
    {
        ...
        // TODO(achulkov2): What should we return here, if anything?
        YT_UNIMPLEMENTED();
    }
    ...
    {
        // TODO(achulkov2): Think a bit more about right value.
        return false;
    }

    TFuture<void> Cancel() override
    {
        // TODO(achulkov2): What else do we need to do?
...
private:
    ...
    // TODO(achulkov2): Move to unit tests for now.
    // Later on we will probably need separate invokers and pollers.
    IInvokerPtr GetInvoker()
    {
        return TDispatcher::Get()->GetWriterInvoker();
    }

    // TODO(achulkov2): We might want to use another poller for S3 interaction.
    IPollerPtr GetPoller()
    {
        return NYT::NBus::TTcpDispatcher::Get()->GetXferPoller();
    }
};
